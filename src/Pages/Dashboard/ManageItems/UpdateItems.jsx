...
const UpdateItems = () => {
    ...
    const {data:loadedRecipe} = useLoaderData();
    const {_id, name, category, recipe, price} = loadedRecipe;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      if (res.data.status === 200) {
        ...
        const menuRes = await axiosSecure.patch(`/menus/${_id}`, menu);
        console.log(menuRes);
        if (menuRes.data.modifiedCount) {
           toast.success("menu Updated successfully");
          return navigate(location.state.pathname)
        }else if(menuRes.data.matchedCount){
           toast.success("nothing for Update");
           return navigate(location.state.pathname)
        } else {
          toast.error("menu not updated");
          return navigate(location.state.pathname)
        }
      }
    } catch (err) {
      toast.error(err.message);
      return navigate(location.state.state.pathname)
    }
  };
  return (
    <div>
